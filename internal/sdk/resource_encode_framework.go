			...
			case reflect.Map:
				...
				if len(fieldVal.MapKeys()) > 0 {
					...
					case reflect.String:
						...
						for iter.Next() {
							attr[iter.Key().String()] = iter.Value().String()
						}
						output[tfschemaTag] = attr
					case reflect.Int, reflect.Int32, reflect.Int64:
						attr := make(map[string]int64)
						for iter.Next() {
							attr[iter.Key().String()] = iter.Value().Int()
						}
						output[tfschemaTag] = attr
					//case reflect.Int32:
					//	attr := make(map[string]int32)
					//	for iter.Next() {
					//		attr[iter.Key().String()] = int32(iter.Value().Int())
					//	}
					//	output[tfschemaTag] = attr
					//case reflect.Int64:
					//	attr := make(map[string]int64)
					...
					case reflect.Float32, reflect.Float64:
						...
						for iter.Next() {
							attr[iter.Key().String()] = iter.Value().Float()
						}
						output[tfschemaTag] = attr
					...
					case reflect.Bool:
						...
						for iter.Next() {
							attr[iter.Key().String()] = iter.Value().Bool()
						}
						output[tfschemaTag] = attr
						//default:
